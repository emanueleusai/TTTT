...
// 
//    TH1F* h_num_GD_trijet   = new TH1F("h_num_GD_trijet","h_num_GD_trijet", 500, 0.0, 500);   
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (topPt_TTbarMassCalc->size() == 4 && genleptonCount == 1){
        allLeptonicWcount++;
        is_genFourTopsOnelepton = true;
    }
    
    if (!is_genMissingDaughter && is_genFourTopsOnelepton){
        ...
        do {
            ...
            std::vector<double> v_flipDeltaR;
            std::vector<int> v_jetIndex;
            for(unsigned int njet = 0; njet < 3; ++njet){
                ...
                v_jetIndex.push_back(idx_minDR);
            ...
            if(bit9set.test(0)*bit9set.test(1)*bit9set.test(2)){
                frst3 = bit9set.test(0)*bit9set.test(1)*bit9set.test(2);
                num_GD_trijet++;
                for(unsigned int idx = 0; idx<3; idx++){
//                     DR_threeConsecutiveMatch->Fill(v_flipDeltaR[idx]);
                    v_123_deltalR.push_back(v_flipDeltaR[idx]);
                }
            }
            else if(bit9set.test(3)*bit9set.test(4)*bit9set.test(5)){
                scnd3 = bit9set.test(3)*bit9set.test(4)*bit9set.test(5);
                num_GD_trijet++;
                for(unsigned int idx = 3; idx<6; idx++){
//                     DR_threeConsecutiveMatch->Fill(v_flipDeltaR[idx]);
                    v_456_deltalR.push_back(v_flipDeltaR[idx]);
                }
            }
            else if(bit9set.test(6)*bit9set.test(7)*bit9set.test(8)){
                thrd3 = bit9set.test(6)*bit9set.test(7)*bit9set.test(8);                
                num_GD_trijet++;
                for(unsigned int idx = 6; idx<9; idx++){
//                     DR_threeConsecutiveMatch->Fill(v_flipDeltaR[idx]);
                    v_789_deltalR.push_back(v_flipDeltaR[idx]);
                }
            }
            else {
                num_BD_trijet++;
//                 for(unsigned int idx = 0; idx<9; idx++){
//                     DR_nonthreeConsecutiveMatch->Fill(v_flipDeltaR[idx]);                    
//                 }
...
// h_num_GD_trijet->GetYaxis()->SetRangeUser(10.,10*(h_num_GD_trijet->GetMaximum()));
// h_num_GD_trijet->Draw();
// h_num_BD_trijet->Draw("SAME");
// c1->SaveAs("numTrijet.png");
// c1->Clear();
// c1->Update();
// 
// 
